...
namespace halx::dynamixel {

class PortHandler : public ::dynamixel::PortHandler {
public:
  PortHandler(peripheral::UartBase &uart) : uart_{uart} { is_using_ = false; }
  ...
  int writePort(uint8_t *packet, int length) override {
    if (!uart_.transmit(packet, length, core::MAX_DELAY)) {
      return 0;
    }
    return length;
  }
...
private:
  halx::peripheral::UartBase &uart_;
